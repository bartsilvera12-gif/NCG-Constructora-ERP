...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertBase = {
      ...
      contacto_persona:     contacto_persona?.trim() || null,
      fecha_alta:           /^\d{4}-\d{2}-\d{2}$/.test(String(fecha_alta ?? "")) ? fecha_alta : null,
      fecha_baja:           /^\d{4}-\d{2}-\d{2}$/.test(String(fecha_baja ?? "")) ? fecha_baja : null,
      regimen_fiscal:       regimen_fiscal?.trim() || null,
      forma_pago:           forma_pago?.trim() || null,
      iban:                 iban?.trim() || null,
      bic_swift:            bic_swift?.trim() || null,
