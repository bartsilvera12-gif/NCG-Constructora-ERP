...
export async function GET(request: NextRequest) {
  ...
  try {
    const { rows, totals } = await fetchLibroVentas(ctx.supabase, ctx.auth.empresa_id, { desde, hasta });
    ...
    return xlsxResponse(buildXlsx("Libro de Ventas", excelRows), `libro-ventas${slugFecha(desde, hasta)}.xlsx`);
  } catch (e) {
