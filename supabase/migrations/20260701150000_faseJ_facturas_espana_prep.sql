...
--
-- Instancia dedicada NCG (schema único `ncgconstructora`). NO tocar public.
--
...
-- fiscal). Sin tocar SIFEN.
--
-- Todas las columnas son nullables + CHECKs enum. Opt-in por factura: si
...
ALTER TABLE ncgconstructora.facturas
  ...
  ADD COLUMN IF NOT EXISTS proyecto_id_ncg        uuid,
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'facturas_tipo_operacion_chk'
      AND conrelid = 'ncgconstructora.facturas'::regclass
  ) THEN
    ALTER TABLE ncgconstructora.facturas
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'facturas_estado_fiscal_chk'
      AND conrelid = 'ncgconstructora.facturas'::regclass
  ) THEN
    ALTER TABLE ncgconstructora.facturas
...
COMMENT ON COLUMN ncgconstructora.facturas.cif_nif_receptor IS
  'CIF / NIF del receptor (España). NULL si la factura no se emite bajo régimen ES.';
COMMENT ON COLUMN ncgconstructora.facturas.estado_fiscal IS
  'Estado ante Hacienda: pendiente / informada / validada / rechazada. NULL = no aplica.';
COMMENT ON COLUMN ncgconstructora.facturas.proyecto_id_ncg IS
  'Obra asociada. FK lax (sin constraint duro) para tolerar borrados de proyectos.';
